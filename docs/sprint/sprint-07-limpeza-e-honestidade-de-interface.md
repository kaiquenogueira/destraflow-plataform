...
Importante para o desenho da decisão: **não existe `middleware.ts`** no repositório (`find . -name middleware.ts` não retorna nada). Os únicos gates de entrada são `requireAdmin` (`admin-auth.ts:5`) e `getTenantContext` (`tenant.ts:21`). `getTenantContext` autentica a sessão e resolve o tenant, mas **não** distingue `USER` de `ADMIN`.
...
Estrutural, **não tratar como vulnerabilidade confirmada**. O modelo de dados é **1 tenant por `databaseUrl`**; a distinção `USER`/`ADMIN` *dentro* de um tenant pode ser intencionalmente indefinida (o `ADMIN` global é gateado por `requireAdmin` em outra camada, nas ações de `/admin`). Hoje, qualquer `crmUser` autenticado com `databaseUrl` definido recebe `TenantContext` completo e executa qualquer ação de tenant. Isso é uma **questão de produto/política**, não um bug a corrigir às cegas.
